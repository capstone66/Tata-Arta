...
async function apiGet<T>(url: string): Promise<T> {
  ...
  if (!res.ok)
    throw new Error(
      (await res.json().catch(() => ({ message: "Gagal" }))).message,
    );
  ...
  const fetchAll = async () => {
    ...
    try {
      const [budgetRes, spendingRes, catData] = await Promise.all([
        apiGet<{ budgets: BudgetItem[] }>(
          `/api/budgets?month=${month}&year=${year}`,
        ),
        apiGet<{ report: SpendingReport[] }>(
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {spending.map((item) => (
            <Card
              key={item.id}
              className="transition-all duration-200 hover:border-primary/20 hover:shadow-sm"
            >
                ...
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Anggaran {formatCurrency(item.budgeted)} &middot; Terpakai{" "}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const b = budgets.find(
                          (x) => x.categoryId === item.category.id,
                        );
